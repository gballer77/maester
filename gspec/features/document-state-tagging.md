...
- **Maester Configuration** ([maester-configuration.md](maester-configuration.md)) — Extends the maester config entry schema with an optional `state` field. The `maester publish` walkthrough prompts per published document for a state choice of `draft`, `canon`, or "file header"; `draft`/`canon` are written as the `state:` field on the entry, and "file header" omits the field so inline state in each file (or the default `draft`) governs.
