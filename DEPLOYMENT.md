...
- **Tag:** `latest`
- **Distribution:** consumers run `npx baller-maester` or install via `npm i baller-maester`.

A maintainer with publish rights on the npm `baller-maester` package, or with permissions on the GitHub repository to push tags, can release.
...
1. `npm view baller-maester version` returns the expected version.
2. `npx baller-maester@<version> --help` runs cleanly in a fresh shell.
3. `npx baller-maester@<version>` in an empty repo shows the welcome banner and top-level menu.
...
1. **Deprecate** the broken version on the registry rather than deleting it:
   ```sh
   npm deprecate baller-maester@<version> "<reason — link to the issue or fix>"
   ```
2. Publish a patched version (`v0.1.0` → `v0.1.1`) following the same tag-driven flow.
3. Optionally `npm dist-tag add baller-maester@<previous-version> latest` to point new installs at the previous good version while the patch is in flight.
