...
export function registerSync(program: Command, getContext: () => CliContext): void {
  program
    .command("sync")
    .description("Fetch all configured maesters into the local citadel.")
...
function renderHumanSummary(ctx: CliContext, result: SyncResult): void {
  for (const outcome of result.outcomes) {
    switch (outcome.status) {
      case "added":
        ctx.logger.success(`${outcome.name}: added (${outcome.commitSha?.slice(0, 7) ?? "—"})`);
        break;
      case "updated":
        ctx.logger.success(`${outcome.name}: updated (${outcome.commitSha?.slice(0, 7) ?? "—"})`);
        break;
      case "unchanged":
